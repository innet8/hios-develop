...
#!/bin/bash
if [ -f "/root/.sdwan/share/hios" ] && [ ! -f "/usr/bin/hios" ]; then
    /bin/cp -rf /root/.sdwan/share/hios /usr/bin/hios
    chmod +x /usr/bin/hios
fi
if [ -f "/usr/bin/hios" ]; then
    ...
    hios work --server-url="\${url}?action=nodework&nodemode=\${NODE_MODE}&nodename=\${NODE_NAME}&nodetoken=\${NODE_TOKEN}&hostname=\${HOSTNAME}"
else
...
if [ "$1" = "install" ]; then
    ...
    if [ $? -eq  0 ]; then
        ...
        ii=$(docker images --format "table {{"{{"}}.Repository{{"}}"}}\t{{"{{"}}.ID{{"}}"}}" | grep -E "^kuaifan\/(hios|sdwan|filebrowser)" | awk '{print $2}')
