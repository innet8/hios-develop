...
check_work() {
    local url=`_wsurl`
    local exist=`ps -ef | grep '/usr/lib/hicloud/hios work' | grep -v 'grep'`
    [ -n "$url" ] && [ -z "$exist" ] && {
        _network
        if [ $? -eq 0 ]; then
            echo "network is blocked, try again 10 seconds"
        else
            nohup /usr/lib/hicloud/hios work --server-url="${url}?action=nodework&nodemode=${NODE_MODE}&nodename=${NODE_NAME}&nodetoken=${NODE_TOKEN}&hostname=${HOSTNAME}" > /dev/null 2>&1 &
        fi
    }
}
